...
def format_plan_text(plan: TradePlan) -> str:
    ...
    lines.append(f"  BB: Upper ${s.bollinger_upper:,.2f} / Lower ${s.bollinger_lower:,.2f} ({s.bollinger_width_pct:.1f}%{' SQZ' if s.bollinger_squeeze else ''})")
    lines.append(f"  OBV: {s.obv:,.0f}  |  ADX (14): {s.adx:.1f} ({s.adx_strength})")
...
def format_plan_markdown(plan: TradePlan) -> str:
    ...
    md.append(f"| Bollinger Upper | ${s.bollinger_upper:,.2f} |")
    md.append(f"| Bollinger Lower | ${s.bollinger_lower:,.2f} |")
    md.append(f"| BB Width | {s.bollinger_width_pct:.1f}% {'(SQUEEZE)' if s.bollinger_squeeze else ''} |")
    md.append(f"| OBV | {s.obv:,.0f} |")
    md.append(f"| ADX (14) | {s.adx:.1f} ({s.adx_strength}) |")
    md.append("")
